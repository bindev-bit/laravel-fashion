            ...
            <div class="flex flex-col items-center sm:px-5 md:flex-row">
                <div class="w-full md:w-1/2">
                    <a href="#_" class="block">
                        <img class="object-cover w-full h-full rounded-lg max-h-64 sm:max-h-96"
                            src="{{ asset('storage/' . $wishlist->products->image_url) }}">
                    </a>
                </div>
                <div class="flex flex-col items-start justify-center w-full h-full py-6 mb-6 md:mb-0 md:w-1/2">
                    <div
                        ...
                        @if ($wishlist->products->category->id == 1)
                            <div
                            ...
                            @elseif ($wishlist->products->category->id == 2)
                                <div
                        ...
                        @endif
                        <span>{{ $wishlist->products->category->name }}</span>
                    </div>
                    <h1 class="text-4xl font-bold leading-none lg:text-5xl xl:text-6xl"><a
                            href="#_">{{ $wishlist->products->name }}</a></h1>
                    <p class="text-sm text-gray-500">{{ $wishlist->products->description }}.</p>
                    <p class="pt-2 text-xl font-bold font-medium">
                        <span class="mx-1">$ </span>
                        <span class="mx-1 text-gray-600">{{ $wishlist->products->price }}</span>
                    </p>
                    <form action="{{ route('wishlists.destroy', $wishlist->id) }}" method="POST">

                        @csrf
                        @method('delete')
                        <input type="hidden" id="id" name="id" value="{{ $wishlist->products->id }}" />
